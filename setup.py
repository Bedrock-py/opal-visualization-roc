"""Pip install script for bedrock.visualization.roc
"""
...
def opalRegistration():
    ...
    algs = ['ROC']
    for alg in algs:
        success = manage_opals("add","visualization","opals.roc.{0}.{0}".format(alg))
        if (success == False):
            success = manage_opals("reload","visualization","opals.roc.{0}.{0}".format(alg))
            if (success == False):
                raise
...
setup(
    ...
    url='https://github.gatech.edu/Bedrock/opal-visualization-roc',
